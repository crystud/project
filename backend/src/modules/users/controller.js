...
export default class UsersController {
  ...
  static async getNoRoleUsers() {
    try {
      const users = await sequelize.query(`SELECT user.id, user.name, user.email, user.address
      FROM users user
          LEFT JOIN teachers teacher ON user.id = teacher.userID
          LEFT JOIN students student ON user.id = student.userID
      WHERE teacher.userID IS NULL AND student.userID IS NULL`, {
        model: Users,
        type: sequelize.QueryTypes.SELECT,
      })
